...
export const composer = async (name) => {

  const { overview, testcases, template, solverTemplate } = problem;
  ...
  try {
    ...
    New Scaffolding:
      + Directory: ${name}
    `);
    const newDir = await generateDir(name);
